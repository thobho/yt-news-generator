...
def get_auto_generated_for_run(run_id: str) -> bool:
    """Check if a run was auto-generated by reading seed.json."""
    run_storage = get_run_storage(run_id)

    if run_storage.exists("seed.json"):
        try:
            content = run_storage.read_text("seed.json")
            data = json.loads(content)
            return data.get("auto_generated", False)
        except (json.JSONDecodeError, Exception):
            pass
    return False


def get_source_info_for_run(run_id: str) -> Optional[dict]:
    """Get source info from seed.json if available."""
    run_storage = get_run_storage(run_id)
    ...
    if run_storage.exists("seed.json"):
        try:
            content = run_storage.read_text("seed.json")
            data = json.loads(content)
            return data.get("source_info")
        except (json.JSONDecodeError, Exception):
            pass
    return None


def _build_run_summary_from_keys(run_id: str, run_keys: set[str], title: Optional[str] = None, auto_generated: bool = False) -> RunSummary | None:
...
async def get_run(run_id: str, tenant: TenantConfig = Depends(storage_dep)):
    ...
    (
        ...
        asyncio.to_thread(_read_json_file, run_storage, "seed.json"),
    )

    # Extract auto_generated and source_info from seed
    auto_generated = False
    source_info = None
    if seed_data:
        auto_generated = seed_data.get("auto_generated", False)
